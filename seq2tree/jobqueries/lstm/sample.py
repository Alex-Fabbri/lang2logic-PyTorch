...
def do_generate(encoder, decoder, enc_w_list, word_manager, form_manager, opt, using_gpu, checkpoint):
    ...
    while head <= len(queue_decode) and head <=100:
        ...
        i_child = 1
        while True:
            prediction, curr_c, curr_h = decoder(prev_word, s[0], s[1])
            s = (curr_c, curr_h)
...
if __name__ == "__main__":
    ...
    with open(args.output, "w") as output:
        ...
        for i in range(len(data)):
            ...
            x = data[i]
            reference = x[1]
            candidate = do_generate(encoder, decoder, x[0], word_manager, form_manager, args, using_gpu, checkpoint)
